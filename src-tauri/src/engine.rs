...
pub enum BlockType { Request, Parse, KeyCheck, Log, RandomString, ConstantString, ConstantList, GetRandomItem, CurrentUnixTime, DateToUnixTime, UnixTimeToDate, UnixTimeToIso8601, Base64Encode, Base64Decode, GenerateCodeVerifier, GenerateCodeChallenge, GenerateState, GenerateNonce, GenerateFirefoxUA, GenerateGuid, GenerateUUID4, TlsRequest, TlsWreq, Hash, ClearCookies, JumpIF, JumpLabel, Script, Replace, UrlEncode, UrlDecode, RandomInteger, ZipLists, BytesToBase64, GenerateKasada, ForgeRockAuth, HmacSign, AesEncrypt, AesDecrypt, Pbkdf2Derive, RsaEncrypt, Base64ToBytes, EncodeHtmlEntities, DecodeHtmlEntities, Delay, RandomUserAgent }
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct ExecutionLog { pub step: String, pub message: String, pub status: String, pub details: Option<RequestDetails>, pub block_id: Option<String> }
...
impl Context {
    pub fn new(proxy: Option<String>) -> Self {
        let mut cb = reqwest::Client::builder().timeout(Duration::from_secs(30)).cookie_store(false).danger_accept_invalid_certs(true).user_agent("Mozilla/5.0 (X11; Linux x86_64; rv:140.0) Gecko/20100101 Firefox/140.0");
...
where F: Fn(ExecutionLog) + Send + Sync + 'static {
    ...
    let log_and_emit = |ctx: &mut Context, step: &str, message: &str, status: &str, details: Option<RequestDetails>, block_id: Option<String>| {
        let log = ExecutionLog { step: step.to_string(), message: message.to_string(), status: status.to_string(), details, block_id };
    ...
    log_and_emit(&mut ctx, "Start", &format!("Starting config: {}", config.name), "Info", None, None);
    ...
    while block_idx < config.blocks.len() {
        ...
        if block.data.get("disabled").and_then(|v| v.as_bool()).unwrap_or(false) {
            log_and_emit(&mut ctx, &format!("{:?}", block.block_type), "Skipped (disabled)", "Info", None, bid);
            block_idx += 1;
            continue;
        }
        ...
        match block.block_type {
            BlockType::RandomUserAgent => {
                let platform = block.data.get("platform").and_then(|v| v.as_str()).unwrap_or("ALL");
                if let Some(ua) = ua_manager.get_random_user_agent(platform) {
                    let var_name = block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("userAgent");
                    ctx.variables.insert(var_name.into(), ua.clone());
                    log_and_emit(&mut ctx, "RandomUserAgent", &format!("Generated UA for {}: {}", platform, ua), "Success", None, bid);
                } else {
                    log_and_emit(&mut ctx, "RandomUserAgent", &format!("No User Agent found for platform: {}", platform), "Error", None, bid);
                }
            }
            BlockType::GenerateCodeVerifier => { let v = generate_random_crypto_string(32); ctx.variables.insert("codeVerifier".into(), v.clone()); log_and_emit(&mut ctx, "GenerateCodeVerifier", &format!("Generated: {}", v), "Success", None, bid); }
            BlockType::GenerateCodeChallenge => { let mut hasher = Sha256::new(); hasher.update(ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or("")).as_bytes()); let c = base64_url_encode(&hasher.finalize()); ctx.variables.insert("codeChallenge".into(), c.clone()); log_and_emit(&mut ctx, "GenerateCodeChallenge", &format!("Generated: {}", c), "Success", None, bid); }
                ...
                log_and_emit(&mut ctx, "GenerateGuid", &format!("{} = {}", var, g), "Success", None, bid);
            }
            BlockType::GenerateUUID4 => {
                ...
                log_and_emit(&mut ctx, "GenerateUUID4", &format!("{} = {}", var, uuid), "Success", None, bid);
            }
            BlockType::ClearCookies => {
                ...
                log_and_emit(&mut ctx, "ClearCookies", "Cookies cleared", "Success", None, bid);
            }
            BlockType::Request => {
                ...
                let force_windows = block.data.get("force_windows").and_then(|v| v.as_bool()).unwrap_or(false);
                let window_size = block.data.get("window_size").and_then(|v| v.as_str()).unwrap_or("");
                ...
                if ctx.current_auto_redirect {
                    let mut ua = "Mozilla/5.0 (X11; Linux x86_64; rv:140.0) Gecko/20100101 Firefox/140.0".to_string();
                    if force_windows {
                        ua = ua.replace("Linux", "Windows").replace("X11", "Windows NT 10.0; Win64; x64");
                    }
                ...
                if let Some(h_val) = block.data.get("headers").and_then(|v| v.as_str()) {
                    for line in h_val.lines() {
                        if let Some((k, v)) = line.split_once(':') {
                            ...
                            if force_windows {
                                val = val.replace("Linux", "Windows").replace("X11", "Windows NT 10.0; Win64; x64");
                            }
                ...
                if force_windows {
                    user_headers.push(("Sec-CH-UA-Platform".to_string(), "\"Windows\"".to_string()));
                }

                if !window_size.is_empty() {
                    let parts: Vec<&str> = window_size.split(',').collect();
                    if parts.len() == 2 {
                        user_headers.push(("Sec-CH-Viewport-Width".to_string(), parts[0].trim().to_string()));
                        user_headers.push(("Sec-CH-Viewport-Height".to_string(), parts[1].trim().to_string()));
                    }
                }
                ...
                loop {
                            ...
                            if let Ok(multipart_fields) = serde_json::from_value::<Vec<MultipartField>>(multipart_fields_val.clone()) {
                                if !multipart_fields.is_empty() {
                                    ...
                                    if let Some(b) = custom_boundary {
                                        ...
                                        request_body_log = "MULTIPART (CUSTOM BOUNDARY)".to_string();
                                    } else {
                                        ...
                                        for field in multipart_fields {
                                            if let Some(filename) = field.filename {
                                                if let Ok(bytes) = BASE64_STANDARD.decode(&field.value) {
                                                    ...
                                                    log_and_emit(&mut ctx, "Request", &format!("Failed to base64 decode multipart field: {}", field.name), "Error", None, bid.clone());
                                                }
                                            } else {
                                                form = form.text(field.name, ctx.replace_vars(&field.value));
                                            }
                                        }
                                        rb = rb.multipart(form);
                                        request_body_log = "MULTIPART FORM DATA".to_string();
                                    }
                                }
                            } else {
                                log_and_emit(&mut ctx, "Request", "Failed to parse multipart fields from block data", "Error", None, bid.clone());
                            }
                        }
                    ...
                    match rb.send().await {
                        ...
                        Err(e) => {
                            log_and_emit(&mut ctx, "Request", &format!("Error: {}", e), "Error", None, bid.clone());
                ...
                if !had_error {
                    ...
                    log_and_emit(&mut ctx, "Request", &format!("Status: {} (redirects: {}, cookies: {:?})", st_code, redirect_count, cookie_names), if final_status >= 200 && final_status < 400 { "Success" } else { "Fail" }, Some(rd), bid);
                }
            }
            BlockType::Parse => {
                ...
                if mode == "JSON" {
                    if let Ok(json) = serde_json::from_str::<Value>(&src) {
                        if let Some(v) = extract_json_value(&json, block.data.get("json_path").and_then(|v| v.as_str()).unwrap_or("")) {
                            ...
                            log_and_emit(&mut ctx, "Parse", &format!("Parsed: {}", v), "Success", None, bid);
                        } else {
                            ctx.variables.insert(var_name.clone(), String::new());
                            log_and_emit(&mut ctx, "Parse", "JSON path not found (continuing)", "Warning", None, bid);
                        }
                    } else {
                        ctx.variables.insert(var_name.clone(), String::new());
                        log_and_emit(&mut ctx, "Parse", "Invalid JSON source (continuing)", "Warning", None, bid);
                    }
                } else if mode == "Regex" {
                        ...
                        Ok(re) => {
                            if recursive {
                                ...
                                if !results.is_empty() {
                                    ...
                                    log_and_emit(&mut ctx, "Parse", &format!("Parsed {} items via Regex", results.len()), "Success", None, bid);
                                } else {
                                    ctx.variables.insert(var_name.clone(), String::new());
                                    log_and_emit(&mut ctx, "Parse", "No Regex matches found", "Warning", None, bid);
                                }
                            } else {
                                if let Some(caps) = re.captures(&src) {
                                    ...
                                    log_and_emit(&mut ctx, "Parse", &format!("Parsed: {}", output), "Success", None, bid);
                                } else {
                                    ctx.variables.insert(var_name.clone(), String::new());
                                    log_and_emit(&mut ctx, "Parse", "Regex match not found", "Warning", None, bid);
                                }
                            }
                        }
                        Err(e) => {
                            ctx.variables.insert(var_name.clone(), String::new());
                            log_and_emit(&mut ctx, "Parse", &format!("Invalid Regex: {}", e), "Error", None, bid);
                        }
                    }
                } else {
                    ...
                    if recursive {
                        ...
                        if !results.is_empty() {
                            ...
                            ctx.variables.insert(var_name.clone(), joined.clone());
                            if capture { ctx.captured_data.push(format!("{} : {}", var_name, joined)); }
                            log_and_emit(&mut ctx, "Parse", &format!("Parsed {} items", results.len()), "Success", None, bid);
                        } else {
                            ctx.variables.insert(var_name.clone(), String::new());
                            log_and_emit(&mut ctx, "Parse", "No matches found (continuing)", "Warning", None, bid);
                        }
                    } else {
                        ...
                        if let Some(start) = src.find(&l) {
                            ...
                            log_and_emit(&mut ctx, "Parse", &format!("Parsed: {}", val), "Success", None, bid);
                        } else {
                            ctx.variables.insert(var_name.clone(), String::new());
                            log_and_emit(&mut ctx, "Parse", "Left string not found (continuing)", "Warning", None, bid);
                        }
                    }
                }
            }
                ...
                if let Ok(chains) = serde_json::from_value::<Vec<Keychain>>(chains_val) {
                    if !chains.is_empty() {
                        ...
                        for c in chains {
                            ...
                            if ok { ctx.bot_status = c.result_status.to_uppercase(); matched = true; let msg = format!("Match: {}", ctx.bot_status); log_and_emit(&mut ctx, "KeyCheck", &msg, "Success", None, bid); break; }
                        }
                        if !matched {
            ...
            BlockType::TlsRequest => {
                ...
                let force_windows = block.data.get("force_windows").and_then(|v| v.as_bool()).unwrap_or(false);
                let window_size = ctx.replace_vars(block.data.get("window_size").and_then(|v| v.as_str()).unwrap_or(""));
                ...
                if force_windows {
                    if let Some(ua) = hs.get_mut("User-Agent") {
                        *ua = ua.replace("Linux", "Windows").replace("X11", "Windows NT 10.0; Win64; x64");
                    }
                    hs.insert("Sec-CH-UA-Platform".to_string(), "\"Windows\"".to_string());
                }

                if !window_size.is_empty() {
                    let parts: Vec<&str> = window_size.split(',').collect();
                    if parts.len() == 2 {
                        hs.insert("Sec-CH-Viewport-Width".to_string(), parts[0].trim().to_string());
                        hs.insert("Sec-CH-Viewport-Height".to_string(), parts[1].trim().to_string());
                    }
                }
                ...
                match forwarder_client.post("http://127.0.0.1:9090/api/forward").header("x-api-key", "my-auth-key-1").json(&payload).send().await {
                    Ok(res) => {
                        ...
                        // Prepare partial RequestDetails in case of failure or raw text
                        ...
                        if let Ok(parsed) = serde_json::from_str::<StructuredTlsApiResponse>(&full_text) {
                            ...
                            rd.response_body = txt;
                            rd.response_headers = parsed.headers;
                            rd.response_cookies = parsed.cookies;
                            rd.response_url = final_url;

                            log_and_emit(&mut ctx, "TlsRequest", &format!("Status: {}", st), if st >= 200 && st < 300 { "Success" } else { "Fail" }, Some(rd), bid);
                        } else {
                            ...
                            log_and_emit(&mut ctx, "TlsRequest", "Forwarder returned non-JSON response, using raw text", "Info", Some(rd), bid);
                        }
                    }
                    Err(e) => {
                        log_and_emit(&mut ctx, "TlsRequest", &format!("Forwarder Error: {}", e), "Error", None, bid);
                        ctx.bot_status = "ERROR".into();
                    }
                }
            }
            BlockType::TlsWreq => {
                ...
                let force_windows = block.data.get("force_windows").and_then(|v| v.as_bool()).unwrap_or(false);
                let window_size = ctx.replace_vars(block.data.get("window_size").and_then(|v| v.as_str()).unwrap_or("1920,1080"));
                ...
                if force_windows {
                    if let Some(ua) = user_headers.get_mut("User-Agent") {
                        *ua = ua.replace("Linux", "Windows").replace("X11", "Windows NT 10.0; Win64; x64");
                    }
                    user_headers.insert("Sec-CH-UA-Platform".to_string(), "\"Windows\"".to_string());
                }

                if !window_size.is_empty() {
                    let parts: Vec<&str> = window_size.split(',').collect();
                    if parts.len() == 2 {
                        user_headers.insert("Sec-CH-Viewport-Width".to_string(), parts[0].trim().to_string());
                        user_headers.insert("Sec-CH-Viewport-Height".to_string(), parts[1].trim().to_string());
                    }
                }
                ...
                match client_result {
                    Ok(client) => {
                        ...
                        loop {
                                        ...
                                        if !frontend_fields.is_empty() {
                                            ...
                                            if let Some(b) = custom_boundary {
                                                ...
                                                for field in frontend_fields {
                                                    if field.is_file {
                                                        if let Ok(bytes) = BASE64_STANDARD.decode(&field.value) {
                                                            let content_type = field.content_type.unwrap_or_else(|| "application/octet-stream".to_string());
                                                            let filename = field.filename.clone().unwrap_or_else(|| "upload".to_string());
                                                            let part = wreq_multipart::Part::bytes(bytes).file_name(filename).mime_str(&content_type).unwrap();
                                                            form = form.part(field.name, part);
                                                        } else {
                                                            log_and_emit(&mut ctx, "TlsWreq", &format!("Failed decode: {}", field.name), "Error", None, bid.clone());
                                                            had_error = true;
                                                        }
                                                    } else {
                                                        form = form.text(field.name, ctx.replace_vars(&field.value));
                                                ...
                                                if !had_error {
                                                    ...
                                                    request_body_log = "MULTIPART".to_string();
                                                }
                                            }
                                        }
                                    }
                            ...
                            if had_error {
                                log_and_emit(&mut ctx, "TlsWreq", "Skipped due to multipart build error", "Error", None, bid.clone());
                                ctx.bot_status = "ERROR".into();
                                break;
                            }
                            ...
                            match req_result {
                                ...
                                Err(e) => {
                                    log_and_emit(&mut ctx, "TlsWreq", &format!("Request Error: {}", e), "Error", None, bid.clone());
                                    ctx.bot_status = "ERROR".into();
                                    had_error = true;
                                    break;
                                }
                        ...
                        if !had_error {
                            ...
                            log_and_emit(&mut ctx, "TlsWreq", &format!("Status: {} ({})", final_status, emulation_str), if final_status >= 200 && final_status < 400 { "Success" } else { "Fail" }, Some(rd), bid);
                        }
                    }
                    Err(e) => {
                        log_and_emit(&mut ctx, "TlsWreq", &format!("Client Build Error: {}", e), "Error", None, bid);
                        ctx.bot_status = "ERROR".into();
                    }
                }
            }
            BlockType::Hash => {
                ...
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("hashed").into(), res.clone());
                log_and_emit(&mut ctx, "Hash", &format!("{}: {}", algo, res), "Success", None, bid);
            }
            BlockType::RandomString => {
                ...
                log_and_emit(&mut ctx, "RandomString", &format!("Generated: {}", result), "Info", None, bid);
            }
            BlockType::ConstantString => {
                let v = ctx.replace_vars(block.data.get("value").and_then(|v| v.as_str()).unwrap_or(""));
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("constString").into(), v.clone());
                log_and_emit(&mut ctx, "ConstantString", &format!("Set: {}", v), "Info", None, bid);
            }
            BlockType::ConstantList => {
                let l = block.data.get("list").and_then(|v| v.as_str()).unwrap_or("").to_string();
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("constList").into(), l);
                log_and_emit(&mut ctx, "ConstantList", "List set", "Info", None, bid);
            }
            BlockType::GetRandomItem => {
                ...
                if let Some(val) = item { let out_var = block.data.get("output_variable").and_then(|v| v.as_str()).unwrap_or("randomItem").to_string(); ctx.variables.insert(out_var, val.clone()); log_and_emit(&mut ctx, "GetRandomItem", &format!("Picked: {}", val), "Success", None, bid); }
            }
            BlockType::CurrentUnixTime => {
                let ts = chrono::Utc::now().timestamp().to_string();
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("unixTime").into(), ts.clone());
                log_and_emit(&mut ctx, "CurrentUnixTime", &format!("Time: {}", ts), "Info", None, bid);
            }
            BlockType::DateToUnixTime => {
                let inp = ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or(""));
                let fmt = block.data.get("format").and_then(|v| v.as_str()).unwrap_or("%Y-%m-%d %H:%M:%S");
                if let Ok(dt) = chrono::NaiveDateTime::parse_from_str(&inp, fmt) { let ts = dt.and_utc().timestamp().to_string(); ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("unixTime").into(), ts.clone()); log_and_emit(&mut ctx, "DateToUnixTime", &format!("Unix: {}", ts), "Success", None, bid); }
            }
            BlockType::UnixTimeToDate => {
                let inp = ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or(""));
                let fmt = block.data.get("format").and_then(|v| v.as_str()).unwrap_or("%Y-%m-%d %H:%M:%S");
                if let Ok(ts) = inp.parse::<i64>() { if let Some(dt) = chrono::DateTime::from_timestamp(ts, 0) { let ds = dt.format(fmt).to_string(); ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("date").into(), ds.clone()); log_and_emit(&mut ctx, "UnixTimeToDate", &format!("Date: {}", ds), "Success", None, bid); } }
            }
            BlockType::UnixTimeToIso8601 => {
                let inp = ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or(""));
                if let Ok(ts) = inp.parse::<i64>() { if let Some(dt) = chrono::DateTime::from_timestamp(ts, 0) { let iso = dt.to_rfc3339(); ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("iso8601").into(), iso.clone()); log_and_emit(&mut ctx, "UnixTimeToIso8601", &format!("ISO: {}", iso), "Success", None, bid); } }
            }
            BlockType::Base64Encode => {
                let e = BASE64_STANDARD.encode(&ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or("")));
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("base64").into(), e);
                log_and_emit(&mut ctx, "Base64Encode", "Encoded", "Info", None, bid);
            }
            BlockType::Base64Decode => {
                if let Ok(b) = BASE64_STANDARD.decode(&ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or(""))) { if let Ok(s) = String::from_utf8(b) { ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("utf8").into(), s); log_and_emit(&mut ctx, "Base64Decode", "Decoded", "Info", None, bid); } }
            }
            BlockType::Log => {
                let msg = ctx.replace_vars(block.data.get("message").and_then(|v| v.as_str()).unwrap_or(""));
                log_and_emit(&mut ctx, "Log", &msg, "Info", None, bid);
            }
            BlockType::GenerateFirefoxUA => {
                let ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:112.0) Gecko/20100101 Firefox/112.0".to_string();
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("userAgent").into(), ua.clone());
                log_and_emit(&mut ctx, "GenerateFirefoxUA", &format!("Generated: {}", ua), "Success", None, bid);
            }
            BlockType::GenerateState => {
                let v = generate_random_crypto_string(16);
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("state").into(), v.clone());
                log_and_emit(&mut ctx, "GenerateState", &format!("Generated: {}", v), "Success", None, bid);
            }
            BlockType::GenerateNonce => {
                let v = generate_random_crypto_string(16);
                ctx.variables.insert(block.data.get("variable").and_then(|v| v.as_str()).unwrap_or("nonce").into(), v.clone());
                log_and_emit(&mut ctx, "GenerateNonce", &format!("Generated: {}", v), "Success", None, bid);
            }
            BlockType::Delay => {
                ...
                log_and_emit(&mut ctx, "Delay", &format!("Waiting {} ms...", ms), "Info", None, bid.clone());
                tokio::time::sleep(Duration::from_millis(ms)).await;
            }
            BlockType::JumpLabel => {
                let label = block.data.get("label").and_then(|v| v.as_str()).unwrap_or("LABEL");
                log_and_emit(&mut ctx, "JumpLabel", &format!("#{}", label), "Info", None, bid);
            }
            BlockType::JumpIF => {
                ...
                if matches {
                    if let Some(&target_idx) = labels.get(target) {
                        log_and_emit(&mut ctx, "JumpIF", &format!("Condition met: {} {} \"{}\", jumping to #{}", source_var, condition, value, target), "Success", None, bid);
                        block_idx = target_idx;
                        continue; // Skip the increment at the end
                    } else {
                        log_and_emit(&mut ctx, "JumpIF", &format!("Label #{} not found!", target), "Error", None, bid);
                    }
                } else {
                    log_and_emit(&mut ctx, "JumpIF", &format!("Condition not met: {} {} \"{}\"", source_var, condition, value), "Info", None, bid);
                }
            }
            BlockType::Script => {
                ...
                match engine.eval_with_scope::<()>(&mut scope, script) {
                    Ok(_) => {
                        ...
                        log_and_emit(&mut ctx, "Script", "Script executed successfully", "Success", None, bid);
                    }
                    Err(e) => {
                        log_and_emit(&mut ctx, "Script", &format!("Script error: {}", e), "Error", None, bid);
                        ctx.bot_status = "ERROR".into();
                    }
                }
            }
            BlockType::Replace => {
                ...
                ctx.variables.insert(var_name.into(), result.clone());
                log_and_emit(&mut ctx, "Replace", &format!("Replaced \"{}\" with \"{}\" -> {}", to_replace, replacement, var_name), "Success", None, bid);
            }
            BlockType::UrlEncode => {
                let inp = ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or(""));
                ...
                log_and_emit(&mut ctx, "UrlEncode", &format!("Encoded: {}", encoded), "Success", None, bid);
            }
            BlockType::UrlDecode => {
                let inp = ctx.replace_vars(block.data.get("input").and_then(|v| v.as_str()).unwrap_or(""));
                ...
                log_and_emit(&mut ctx, "UrlDecode", &format!("Decoded: {}", decoded), "Success", None, bid);
            }
            BlockType::RandomInteger => {
                ...
                ctx.variables.insert(var_name.clone(), val.to_string());
                log_and_emit(&mut ctx, "RandomInteger", &format!("{} = {}", var_name, val), "Success", None, bid);
            }
            BlockType::ZipLists => {
                ...
                log_and_emit(&mut ctx, "ZipLists", &format!("Zipped {} items into {}", zipped.len(), var_name), "Success", None, bid);
            }
            BlockType::BytesToBase64 => {
                ...
                ctx.variables.insert(var_name.clone(), result);
                log_and_emit(&mut ctx, "BytesToBase64", &format!("Encoded to Base64 in {}", var_name), "Success", None, bid);
            }
            BlockType::GenerateKasada => {
                let version = ctx.replace_vars(block.data.get("version").and_then(|v| v.as_str()).unwrap_or("j-1.2.139"));
                
                let mut rng = rand::rng();
                let now = chrono::Utc::now().timestamp_millis();
                
                // st: starting time (approx 20-30s before now)
                let st = now - rng.random_range(20000..30000);
                // rst: response starting time (approx 100-300ms after st)
                let rst = st + rng.random_range(100..300);
                // workTime: execution time (approx 10-20s after st)
                let work_time = st + rng.random_range(10000..20000);
                
                let id: String = (0..32).map(|_| {
                    let hex_chars = "0123456789abcdef";
                    hex_chars.chars().nth(rng.random_range(0..16)).unwrap()
                }).collect();
                
                let duration = rng.random_range(30..100);
                let d = rng.random_range(150..250);
                let answers = vec![rng.random_range(1..20), rng.random_range(1..5)];
                
                // Construct CD (Telemetry)
                ...
                let cd_str = cd_json.to_string();
                
                // Construct CT (Client Token - Long Random String)
                let ct_len = 160;
                let ct_chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
                let ct: String = (0..ct_len).map(|_| {
                    ct_chars.chars().nth(rng.random_range(0..ct_chars.len())).unwrap()
                }).collect();
                
                // Construct H (Hash - Random Base64)
                let mut h_bytes = [0u8; 20];
                for i in 0..20 { h_bytes[i] = rng.random(); }
                let h = BASE64_STANDARD.encode(h_bytes);
                
                // Insert into variables
                ctx.variables.insert("KPSDK_CD".into(), cd_str.clone());
                ctx.variables.insert("KPSDK_CT".into(), ct.clone());
                ctx.variables.insert("KPSDK_H".into(), h.clone());
                ctx.variables.insert("KPSDK_V".into(), version.clone());
                
                log_and_emit(&mut ctx, "GenerateKasada", &format!("Generated Kasada headers (v: {})", version), "Success", None, bid);
            }
            BlockType::ForgeRockAuth => {
                ...
                if let Ok(mut json) = serde_json::from_str::<serde_json::Value>(&source_json) {
                                        ...
                                        if let Some(js_val) = output.get("value").and_then(|v| v.as_str()) {
                                            if js_val.contains("var work =") {
                                                ...
                                                if !work.is_empty() && difficulty > 0 {
                                                    ...
                                                    log_and_emit(&mut ctx, "ForgeRockAuth", &format!("Solved PoW (Work: {}, Diff: {}): Nonce: {}", work, difficulty, solved_nonce), "Info", None, bid.clone());
                                                }
                                            }
                                        }
                                    }
                    ...
                    if let Some(callbacks) = json.get_mut("callbacks").and_then(|c| c.as_array_mut()) {
                        ...
                        log_and_emit(&mut ctx, "ForgeRockAuth", &format!("Payload generated in <{}> (PoW: {})", out_var, if solved_nonce.is_empty() { "No" } else { "Yes" }), "Success", None, bid);
                    } else {
                        log_and_emit(&mut ctx, "ForgeRockAuth", "No callbacks found", "Error", None, bid.clone());
                        ctx.bot_status = "ERROR".into();
                    }
                } else {
                    ...
                    log_and_emit(&mut ctx, "ForgeRockAuth", &err_msg, "Error", None, bid.clone());
                    ctx.bot_status = "ERROR".into();
                }
            }
            BlockType::HmacSign => {
                ...
                ctx.variables.insert(var_name.into(), result.clone());
                log_and_emit(&mut ctx, "HmacSign", &format!("HMAC-{} ({}): {}", algo, output_format, result), "Success", None, bid);
            }
            BlockType::AesEncrypt => {
                ...
                match result {
                    Ok(encrypted) => {
                        ...
                        log_and_emit(&mut ctx, "AesEncrypt", &format!("Encrypted ({}): {}", output_format, if encrypted.len() > 50 { format!("{}...", &encrypted[..50]) } else { encrypted }), "Success", None, bid);
                    }
                    Err(e) => {
                        log_and_emit(&mut ctx, "AesEncrypt", &format!("Error: {}", e), "Error", None, bid);
                        ctx.bot_status = "ERROR".into();
                    }
                }
            }
            BlockType::AesDecrypt => {
                ...
                match result {
                    Ok(decrypted) => {
                        ...
                        log_and_emit(&mut ctx, "AesDecrypt", &format!("Decrypted: {}", if decrypted.len() > 50 { format!("{}...", &decrypted[..50]) } else { decrypted }), "Success", None, bid);
                    }
                    Err(e) => {
                        log_and_emit(&mut ctx, "AesDecrypt", &format!("Error: {}", e), "Error", None, bid);
                        ctx.bot_status = "ERROR".into();
                    }
                }
            }
            BlockType::Pbkdf2Derive => {
                ...
                let result = match output_format {
                    "base64" => BASE64_STANDARD.encode(&derived_key),
                    "base64url" => BASE64_URL_SAFE_NO_PAD.encode(&derived_key),
                    _ => hex::encode(&derived_key),
                };

                ctx.variables.insert(var_name.into(), result.clone());
                log_and_emit(&mut ctx, "Pbkdf2Derive", &format!("PBKDF2-HMAC-{} ({}): {} ({} iterations, {} bytes)", algo, output_format, if result.len() > 32 { format!("{}...", &result[..32]) } else { result }, iterations, key_length), "Success", None, bid);
            }
            BlockType::RsaEncrypt => {
                ...
                match (|| -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
                    ...
                    Ok(result) => {
                        ctx.variables.insert(var_name.into(), result.clone());
                        log_and_emit(&mut ctx, "RsaEncrypt", &format!("Encrypted: {}...", &result[..32.min(result.len())]), "Success", None, bid);
                    }
                    Err(e) => {
                        log_and_emit(&mut ctx, "RsaEncrypt", &format!("Error: {}", e), "Error", None, bid);
                        ctx.bot_status = "ERROR".into();
                    }
                }
            }
            BlockType::Base64ToBytes => {
                ...
                match BASE64_STANDARD.decode(&input) {
                    Ok(bytes) => {
                        ...
                        log_and_emit(&mut ctx, "Base64ToBytes", &format!("Decoded to hex: {}...", &hex_str[..32.min(hex_str.len())]), "Success", None, bid);
                    }
                    Err(e) => {
                        log_and_emit(&mut ctx, "Base64ToBytes", &format!("Error: {}", e), "Error", None, bid);
                        ctx.bot_status = "ERROR".into();
                    }
                }
            }
            BlockType::EncodeHtmlEntities => {
                ...
                let encoded = input
                    ...
                    .replace('\'', "&#x27;");
                ctx.variables.insert(var_name.into(), encoded.clone());
                log_and_emit(&mut ctx, "EncodeHtmlEntities", &format!("Encoded: {}", if encoded.len() > 50 { format!("{}...", &encoded[..50]) } else { encoded }), "Success", None, bid);
            }
            BlockType::DecodeHtmlEntities => {
                ...
                ctx.variables.insert(var_name.into(), decoded.clone());
                log_and_emit(&mut ctx, "DecodeHtmlEntities", &format!("Decoded: {}", if decoded.len() > 50 { format!("{}...", &decoded[..50]) } else { decoded }), "Success", None, bid);
            }
    ...
    log_and_emit(&mut ctx, "End", &format!("Bot ended: {}", final_status), &final_status, None, None);
